...
def check_tx_nist256(transaction: bytes, der_signature: bytes, public_key: bytes) -> None:
...
def serialize(cla: int, ins: int, p1: int = 0, p2: int = 0, cdata: bytes = b"") -> bytes:
    header: bytes = struct.pack("BBBBB", cla, ins, p1, p2, len(cdata))  # add Lc to APDU header
...
def get_public_key(backend: BackendInterface, bip44_path: str) -> bytes:
    packed = serialize(
        ...
        p1=0x00,
        p2=0x00,
        cdata=pack_derivation_path(bip44_path)[1:],
    )  # No length prefix

    return backend.exchange_raw(packed).data


def get_signed_public_key_and_validate(backend: BackendInterface, bip44_path: str) -> bytes:
    packed = serialize(
        cla=CLA,
...
def sign_tx(scenario_navigator: NavigateWithScenario, tx: bytes, do_navigate: bool) -> RAPDU:
    offset = 0
    ...
    while offset != len(tx):
        ...
        if (offset + len(chunk)) == len(tx):
            with backend.exchange_async(CLA, INS_SIGN, P1_LAST, 0x00, chunk):
                if do_navigate:
                    scenario_navigator.review_approve(custom_screen_text=custom_text)
                pass
...
def sign_and_validate(scenario_navigator: NavigateWithScenario, tx: bytes) -> None:
    # Get public key
    backend = scenario_navigator.backend
