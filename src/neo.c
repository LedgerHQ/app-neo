 ...
 * transaction types.
 *
 * Currently only Claim and Contract are tested, as they are the only ones supported by the current
 * wallets.
 */
enum TX_TYPE {
    TX_MINER = 0x00,
 ...
 *
 * currently there's no support in wallets for adding attributes to a contract, but the types are as
 * listed below.
 */
...
/** GAS's asset id. */
static const char GAS_ASSET_ID[] =
...
/** reads bytes out of src, converts each byte into two hex characters, and writes the hex
 * characters into dest. Only converts enough characters to fill dest_len. */
...
static bool is_asset_id(const unsigned char *asset_id, const char *asset_id_hex) {
    for (int asset_id_ix = ASSET_ID_LEN - 1, asset_id_hex_ix = 0; asset_id_ix >= 0;
         asset_id_ix--, asset_id_hex_ix += 2) {
